        ...
        <sql:setDataSource var = "ds" driver = "org.apache.derby.jdbc.ClientDriver"
         ...
         user = "ash"  password = "ash@123"/>
          <sql:query dataSource = "${ds}" var = "res">
              SELECT * from workallocated where employeeid='<%= request.getParameter("id")%>' order by status desc
             ...
             <tbody>
                      <c:forEach var = "row" items = "${res.rows}">
                          <tr>
                              ...
                              <td>${row.email}</td>
                              <td>${row.status}</td>
                              <td><a href='pwork.jsp?d=${row.eventno}&id=${row.employeeid}'>your work</a></td>
                              <td><a href='editimgtable.jsp?no=${row.eventno}'>edit</a></td>
                          </tr>
                      </c:forEach>
             </tbody>
         </table>
